...
// Not original source - modified for RGB
...
function GameBoyCore(canvas, ROMImage) {
	...
	var testCanvas = $('<canvas></canvas>')[0].getContext('2d');
	this.drawRatio = (window.devicePixelRatio || 1) / (	testCanvas.webkitBackingStorePixelRatio ||	//for HiDPI screens
                            							testCanvas.mozBackingStorePixelRatio ||
                            							testCanvas.msBackingStorePixelRatio ||
                            							testCanvas.oBackingStorePixelRatio ||
                            							testCanvas.backingStorePixelRatio || 1);
...
GameBoyCore.prototype.strippedSaveState = function () {
	return [
		this.inBootstrap,
		this.registerA,
		this.FZero,
...
GameBoyCore.prototype.returnFromState = function (returnedFrom) {
	...
	this.frameBuffer = state[index++];  //this.toTypedArray(state[index++], "int32"); // Performs faster as an untyped array. See note in initMemory function.
...
GameBoyCore.prototype.initMemory = function () {
	...
	this.frameBuffer = this.fillArray(23040, 0xF8F8F8); 		//Used to be an int32 typed array because there was an 8-bit R, G, and B value in each element (giving 24 bits per element). However, this performs faster as an untyped array. Need to try with other arrays as well. Note that uint32 is also slower.
...
GameBoyCore.prototype.getTypedArray = function (length, defaultValue, numberType) {
	try {
		...
		console.warn("Could not convert an array to a typed array for arguments length='"+length+"', defaultValue='"+defaultValue+"', numberType='"+numberType+"'. Error message: " + error.message);
		var arrayHandle = [];
		var index = 0;
		while (index < length) {
			arrayHandle[index++] = defaultValue;
...
GameBoyCore.prototype.fillArray = function (length, defaultValue) {
	var arrayHandle = [];
	var index = 0;
	while (index < length) {
		arrayHandle[index++] = defaultValue;
